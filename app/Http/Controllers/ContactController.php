...
class ContactController extends Controller
{
   ...
   $stages = \DB::table('users')->join("model_has_roles", "model_has_roles.model_id", "=", "users.id")->join("roles", "roles.id", "=", "model_has_roles.role_id")->where([['roles.name', '=', 'Super Admin'],['users.id', '=', $id2]])->count();   
    ...
    $contact = \DB::table('contact')->where('contact.assigned_id', '!=' ,0)->get();   
        ...
        return view('contact.contactlist',compact('contact','stag'));
    }
    ...
    public function listnote2(Request $request)
    {
       ...
       $subject = $input['subject']; 
       $description = $input['description']; 
        \DB::table('list_note')->insert(['contact_id'=>$contactid, 'subject'=>$subject,'description'=>$description,'created_by'=>$created_by,'created_at'=>date('Y-m-d H:i:s')]);
        return Redirect::back();
                        

    }
    public function listnote3(Request $request)
    {
       ...
       $subject = $input['subject']; 
       $description = $input['description']; 
        \DB::table('list_note')->insert(['contact_id'=>$contactid, 'sales'=>1,'subject'=>$subject,'description'=>$description,'created_by'=>$created_by,'created_at'=>date('Y-m-d H:i:s')]);
        return Redirect::back();
